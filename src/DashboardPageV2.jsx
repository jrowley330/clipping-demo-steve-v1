...
export default function DashboardsPageV2() {
  ...
  // Reset filters when switching environments
  useEffect(() => {
    setSummaryMonth("all");
    setSummaryClipper("all");

    setDetailsMonth("all");
    setDetailsWeekOf("all");
    setDetailsClipper("all");
  }, [clientId]);
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/clipper-summary?clientId=${encodeURIComponent(clientId)}`);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/clipper-details?clientId=${encodeURIComponent(clientId)}`);
