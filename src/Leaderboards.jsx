 ...
 *
 * NOTE: Placeholder/mock data REMOVED. This page now fetches real data from API.
 */
...
export default function Leaderboards() {
  ...
  // Filters (real)
  // weekEndParam: sent to API (optional). Keep blank to get latest.
  // weekEndDisplay: what we show after API returns latest week_end_date
  const [weekEndParam, setWeekEndParam] = useState("");
  const [weekEndDisplay, setWeekEndDisplay] = useState("");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        qs.set("limit", "50");
        if (weekEndParam) qs.set("weekEnd", weekEndParam);

        const res = await fetch(`${API_BASE_URL}/leaderboards?${qs.toString()}`);
        if (!res.ok) {
          const txt = await res.text().catch(() => "");
          throw new Error(txt || `Leaderboards API error (${res.status})`);
        }

        const json = await res.json();

        const normalized = (Array.isArray(json?.rows) ? json.rows : []).map((r, i) => ({
          id: String(r.clipper_id ?? `clipper_${i}`),
          name: String(r.clipper_name ?? `Clipper ${i + 1}`),
  ...
  const computed = useMemo(() => {
    const base = rowsApi.map((r) => {
      ...
      // Cadence per day (week window assumed = 7 days)
    ...
    const values = base.map(metricAccessor);
    const minV = values.length ? Math.min(...values) : 0;
    const maxV = values.length ? Math.max(...values) : 0;
    const range = Math.max(1e-9, maxV - minV);

    const withRating = base.map((r) => {
      const v = metricAccessor(r);
      const t = values.length ? (v - minV) / range : 0;
      const rating = 1 + t * 4;
      return { ...r, rating };
    });

    const sorted = [...withRating].sort((a, b) => metricAccessor(b) - metricAccessor(a));
    const podium = sorted.slice(0, 3);
    const rest = sorted.slice(3);

    const topViews = [...withRating].sort((a, b) => b.views - a.views)[0];
    const mostVideos = [...withRating].sort((a, b) => b.videos - a.videos)[0];
    const bestVpp = [...withRating].sort((a, b) => b.viewsPerVideo - a.viewsPerVideo)[0];
    const bestE1k = [...withRating].sort((a, b) => b.engPer1k - a.engPer1k)[0];

    const totalViews = withRating.reduce((s, r) => s + Number(r.views || 0), 0);
    const totalVideos = withRating.reduce((s, r) => s + Number(r.videos || 0), 0);
    const totalLikes = withRating.reduce((s, r) => s + Number(r.likes || 0), 0);
    const totalComments = withRating.reduce((s, r) => s + Number(r.comments || 0), 0);
  ...
  const copySummary = async () => {
    ...
    computed.podium.forEach((p, idx) => {
      ...
      lines.push(
        `${medal} ${p.name} — ${formatNumber(p.views)} views · ${formatNumber(p.likes)} likes · ${formatNumber(
          p.comments
        )} comments · ${formatNumber(p.videos)} videos`
      );
    });

    lines.push("");
    lines.push(
      `Totals: ${formatNumber(computed.totalViews)} views · ${formatNumber(computed.totalLikes)} likes · ${formatNumber(
        computed.totalComments
      )} comments · ${formatNumber(computed.totalVideos)} videos`
    );
    ...
    lines.push(
      `Best engagement/1K views: ${computed.bestE1k?.name || "—"} (${(computed.bestE1k?.engPer1k || 0).toFixed(1)})`
    );
    lines.push(
      `Most videos: ${computed.mostVideos?.name || "—"} (${formatNumber(computed.mostVideos?.videos || 0)})`
    );
    ...
    >
      ...
      <div style={{ flex: 1, position: "relative", zIndex: 3 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
            ...
            <span style={{ fontSize: 13, opacity: 0.7 }}>
              Week of {headerWeekLabel} · Ranked by{" "}
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={printToPdf} style={topActionBtnStyle()} title="Print / Save as PDF">
              Print / Save PDF
            </button>
        ...
        <div ref={boardRef} className="print-area">
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Pill tone="blue">
                ...
                <span style={{ fontWeight: 900 }}>{loading ? "—" : formatNumber(computed.totalViews)}</span>
              </Pill>
              <Pill tone="green">
                <span style={{ opacity: 0.8 }}>Likes</span>
                <span style={{ fontWeight: 900 }}>{loading ? "—" : formatNumber(computed.totalLikes)}</span>
              </Pill>
              <Pill tone="pink">
                <span style={{ opacity: 0.8 }}>Comments</span>
                <span style={{ fontWeight: 900 }}>{loading ? "—" : formatNumber(computed.totalComments)}</span>
              </Pill>
              <Pill tone="neutral">
                <span style={{ opacity: 0.8 }}>Videos</span>
                <span style={{ fontWeight: 900 }}>{loading ? "—" : formatNumber(computed.totalVideos)}</span>
              </Pill>
            </div>
          </div>
          ...
          >
            <PodiumCard place={2} person={computed.podium[1]} rankBy={rankBy} loading={loading} />
            <PodiumCard place={1} person={computed.podium[0]} rankBy={rankBy} isFirst loading={loading} />
            <PodiumCard place={3} person={computed.podium[2]} rankBy={rankBy} loading={loading} />
          </div>
          ...
          >
            <MiniCard
              title="Most views"
              tone="gold"
              main={loading ? "Loading..." : computed.topViews?.name || "—"}
              sub={loading ? "—" : `${formatNumber(computed.topViews?.views)} views`}
            />
            <MiniCard
              title="Most videos posted"
              tone="blue"
              main={loading ? "Loading..." : computed.mostVideos?.name || "—"}
              sub={loading ? "—" : `${formatNumber(computed.mostVideos?.videos)} videos`}
            />
            <MiniCard
              title="Best views per video"
              tone="silver"
              main={loading ? "Loading..." : computed.bestVpp?.name || "—"}
              sub={
                loading
                  ? "—"
                  : `${formatNumber(Math.round(computed.bestVpp?.viewsPerVideo || 0))} avg views/video`
              }
            />
            <MiniCard
              title="Best engagement / 1K views"
              tone="green"
              main={loading ? "Loading..." : computed.bestE1k?.name || "—"}
              sub={loading ? "—" : `${(computed.bestE1k?.engPer1k || 0).toFixed(1)} per 1K views`}
            />
          </div>
          ...
          >
            ...
            >
              <div>
                <div style={{ fontSize: 14, fontWeight: 900, letterSpacing: 0.2 }}>Full Rankings</div>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {computed.rest.map((r, idx) => (
                    <RankRow key={r.id} rank={idx + 4} row={r} metricValue={computed.metricAccessor(r)} />
                  ))}
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button onClick={copySummary} style={topActionBtnStyle()} disabled={loading}>
                  Copy text
                </button>
...
function PodiumCard({ place, person, rankBy, isFirst, loading }) {
  const tone = place === 1 ? "gold" : place === 2 ? "silver" : "bronze";

  const bgGlow =
    place === 1 ? "rgba(250,204,21,0.18)" : place === 2 ? "rgba(226,232,240,0.14)" : "rgba(251,146,60,0.14)";

  const safePerson = person || {};

  const metricValue =
    rankBy === "views"
      ? formatNumber(safePerson?.views)
      : rankBy === "videos"
      ? formatNumber(safePerson?.videos)
      : rankBy === "vpp"
      ? formatNumber(Math.round(safePerson?.viewsPerVideo || 0))
      : (safePerson?.engPer1k || 0).toFixed(1);

  const metricLabel =
    rankBy === "views" ? "Views" : rankBy === "videos" ? "Videos" : rankBy === "vpp" ? "Views/Video" : "Eng/1K";
  ...
  return (
    <div
      style={{
        ...
        border: "1px solid rgba(148,163,184,0.25)",
        boxShadow: isFirst ? "0 30px 100px rgba(0,0,0,0.85)" : "0 22px 70px rgba(0,0,0,0.78)",
        position: "relative",
        overflow: "hidden",
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 2 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.12, opacity: 0.7 }}>
              ...
              >
                {loading ? "Loading..." : safePerson?.name || "—"}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
            <Pill tone={tone}>
              ...
              <span style={{ fontWeight: 900 }}>{loading ? "—" : metricValue}</span>
            </Pill>
            <Stars rating={safePerson?.rating || 0} />
          </div>
        </div>
        ...
        <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
          ...
          <MiniStat label="Videos" value={loading ? "—" : formatNumber(safePerson?.videos)} />
        </div>
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Pill>
            ...
            <span style={{ fontWeight: 900 }}>
              {loading ? "—" : formatNumber(Math.round(safePerson?.viewsPerVideo || 0))}
            </span>
          </Pill>
          <Pill>
            <span style={{ opacity: 0.75 }}>Eng/1K</span>
            <span style={{ fontWeight: 900 }}>{loading ? "—" : (safePerson?.engPer1k || 0).toFixed(1)}</span>
          </Pill>
          <Pill>
            <span style={{ opacity: 0.75 }}>Cadence</span>
            <span style={{ fontWeight: 900 }}>{loading ? "—" : (safePerson?.cadence || 0).toFixed(1)}/day</span>
          </Pill>
        </div>
      </div>
    </div>
    ...
    <div
      style={{
        ...
        background: isTop
          ...
          : "rgba(15,23,42,0.92)",
        border: isTop ? "1px solid rgba(255,255,255,0.14)" : "1px solid rgba(148,163,184,0.30)",
        boxShadow: isTop ? "0 18px 45px rgba(0,0,0,0.82)" : "0 14px 30px rgba(15,23,42,0.85)",
        display: "flex",
        alignItems: "center",
        justifyContent: "space-between",
        gap: 12,
